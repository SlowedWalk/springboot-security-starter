...
import tech.hidetora.securityStarter.utils.CacheNames;

import java.time.Duration;

@EnableCaching
@Configuration
public class CahcheConfig {
    @Value("${cache.config.entryTtl:60}")
    private int entryTtl;

    @Value("${cache.config.usersTtl:60}")
    ...
    public RedisCacheManagerBuilderCustomizer redisCacheManagerBuilderCustomizer() {
        return builder -> {
            ...
            builder.withCacheConfiguration(CacheNames.USERS, countryNamesConf);
        };
    }
}
